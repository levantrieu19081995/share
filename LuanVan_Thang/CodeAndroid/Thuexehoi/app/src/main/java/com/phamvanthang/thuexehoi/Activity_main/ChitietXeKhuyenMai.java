...
import com.phamvanthang.thuexehoi.Adapter.Adapter_xe_lienquan_khuyenmai;
...
public class ChitietXeKhuyenMai extends AppCompatActivity {
    ...
    private Adapter_xe_lienquan_khuyenmai adapter_xe_lienquan_khuyenmai;
    private List<Xe_lienquan_khuyenmai> xe_lienquan_khuyenmais;

    String urlserver_thongthinchitietxe_khuyenmai = Class_bien_duongdan.ip_server + "/get_thongtinchitietxe_khuyenmai.php";
    String urlserver_hinhSlider = Class_bien_duongdan.ip_server + "/get_hinhslider.php";
    String urlserver_danhsachxelienquan = Class_bien_duongdan.ip_server + "/get_danhsachxelienquan_khuyenmai.php";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView( R.layout.activity_chitiet_xe_khuyenmai );
        Mapping();
        slider();
        danhsach_xe_lienquan();
        lay_thongtingiaxe( urlserver_thongthinchitietxe_khuyenmai );
        onclick();
        DatDiem();
    }
    //___________________________________________________________________________________________//
    ...
    private void Mapping() {
        ...
        txt_giagiam = (TextView) findViewById( R.id.txt_giagiam );
        ...
        System.out.println( "______________________ ma_xe  khuyen mãi: " + ma_xe );
    }
                    ...
                    public void onResponse(String response) {
                        try {
                            ...
                            for (int i = 0; i < jsonArray.length(); i++) {
                                ...
                                if (op.getString( "phantram_lx" ).equals( null ) || op.getString( "phantram_lx" ).equals( "" )) {
                                    txt_giagiam.setText( "Chưa có xự kiện !" );
                                    gia_xethue = Double.valueOf( Integer.parseInt( op.getString( "gia_lx" ) ) );
                                    txt_chiphi.setText( gia_xethue + "đ X " + 1 + " ngày" );
                                    txt_so_tienTongcong.setText( gia_xethue * 1 + "K" );
                                } else {
                                    txt_giagiam.setText( "" + tinhphantram( Double.parseDouble( op.getString( "phantram_lx" ) ), Integer.parseInt( op.getString( "gia_lx" ) ) ) );
                                    txt_chiphi.setText( tinhphantram( Double.parseDouble( op.getString( "phantram_lx" ) ), Integer.parseInt( op.getString( "gia_lx" ) ) ) + "đ X " + 1 + " ngày" );
                                    gia_xethue = Double.valueOf( tinhphantram( Double.parseDouble( op.getString( "phantram_lx" ) ), Integer.parseInt( op.getString( "gia_lx" ) ) ) );
                                    txt_so_tienTongcong.setText( gia_xethue * 1 + "K" );
                                }
    ...
    private void danhsach_xe_lienquan() {
        ...
        xe_lienquan_khuyenmais = new ArrayList<>();
        danhsach_xe_lienquans();
        adapter_xe_lienquan_khuyenmai = new Adapter_xe_lienquan_khuyenmai( getApplicationContext(), xe_lienquan_khuyenmais );
        rcv_xelienquan.setAdapter( adapter_xe_lienquan_khuyenmai );
        adapter_xe_lienquan_khuyenmai.notifyDataSetChanged();
    }
        ...
        StringRequest stringRequest = new StringRequest( Request.Method.POST, urlserver_danhsachxelienquan,
                new Response.Listener<String>() {
                    ...
                    public void onResponse(String response) {
                        try {
                            JSONArray jsonArray = new JSONArray( response );
                            xe_lienquan_khuyenmais.clear();
                            for (int i = 0; i < jsonArray.length(); i++) {
                                JSONObject op = jsonArray.getJSONObject( i );
                                xe_lienquan_khuyenmais.add( new Xe_lienquan_khuyenmai(
                                        op.getString( "hinh" ),
                                        op.getString( "loaixe" ),
                                        op.getString( "gia" ),
                                        op.getString( "diachi" ),
    ...
    private void AnhXa_dialog_dangnhap() {
        dialog_dangnhap = new Dialog( ChitietXeKhuyenMai.this, android.R.style.Theme_Black_NoTitleBar_Fullscreen );
    ...
    private void LayThongTinCaNhan(final String matk) {
        ...
        StringRequest stringRequest = new StringRequest( Request.Method.POST, url_laythongtinnguoidung,
                ...
                new Response.ErrorListener() {
                    @Override
                    public void onErrorResponse(VolleyError error) {
                        Toast.makeText( ChitietXeKhuyenMai.this,  "Server đang gập sự cố...!", Toast.LENGTH_SHORT ).show();
                    }
                } ) {
            @Override
            protected Map<String, String> getParams() throws AuthFailureError {
    ...
    private void thongTin_DatXe(String Maxe, String MaKh, String ngay_gioBD, String ngay_gioKT, String chiPhi,String ten_xe, String hinh_xe, String diachi_xe){
        if(ngay_gioBD.equals( null )||ngay_gioKT.equals( null )|| ngay_gioBD.equals( "" )||ngay_gioKT.equals( "" )){
            ...
            Intent i = new Intent( ChitietXeKhuyenMai.this, Datxe.class );
    ...
    private void tinhSoNgay() throws ParseException {
        ...
        cal2.setTime( date );
        //System.out.println( "__________________Số ngày là =  " + daysBetween( cal1.getTime(), cal2.getTime() ) );
    }
    ...
    //hàm tính số ngày giữa 2 ngày
    @RequiresApi(api = Build.VERSION_CODES.O)
    private void tinhSoNgay2(
            int yearbd, int monthbd, int dayofMonthbd, int hourbd, int minutebd, int secondbd,
            int yearkt, int monthkt, int dayofMonthkt, int hourkt, int minutekt, int secondkt) throws ParseException {
